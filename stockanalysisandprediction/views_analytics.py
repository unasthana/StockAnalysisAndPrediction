...
def getAnalyticDataApi(request, stock_ticker, analytic):
    ...
    response = prepare_response(getAnalyticData(analytic, stock_ticker, time, ma_analytic, ma_window))
    keys = ["result_df", "max_result", "max_date", "min_result", "min_date", "avg_result"]
    return JsonResponse(dict(zip(keys, response)), safe=False)
...
def getMovingAveragesApi(request, stock_ticker, analytic):
    ...
    time = request.GET.get('time', 'all_time')
    response = prepare_response(getMovingAverages(stock_ticker, analytic, ma_window, time))
    keys = ["result_df", "max_result", "max_date", "min_result", "min_date", "avg_result"]
    return JsonResponse(dict(zip(keys, response)), safe=False)
